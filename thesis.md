...
As the analysis of the movie data relies heavily on the use of second-party and third-party data, then it is also imperitive to address the way that certain data risks can affect the three subgroups. For the production, rating, and sales subgroups, the use of a third-party source could potentially cause an indirect 
To mitigate the risk associated with using second-party and third-party data, this research will 
